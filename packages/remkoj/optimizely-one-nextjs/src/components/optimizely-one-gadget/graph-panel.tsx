...
export const GraphPanel : FunctionComponent<GraphPanelProps> = ({ servicePrefix = '/api/me', refreshInterval = 30000}) => 
{
    ...
    return <>
        <dl className='oo-text-sm'>
            ...
            <dd>
                <ul>
                    { (data?.hooks || []).map(hook => <li key={`webhook-${ hook.url }`}>{ hook.method && <span className='oo-text-xs'>[{ hook.method }] </span>}{ hook.url }{ hook.hasQueryParams && <>?...</>}</li>)} 
                </ul>
            </dd>
            <dt className='oo-font-bold oo-pt-1'>Exposing content from:</dt>
            <dd>
                <ul>
        ...
        <p className='oo-pt-4 oo-text-sm'><Link className='oo-underline oo-text-blue-800' href="https://cg.optimizely.com/app/portal/" target='_new'>Login to Optimizely Graph</Link></p>
    </>
}
