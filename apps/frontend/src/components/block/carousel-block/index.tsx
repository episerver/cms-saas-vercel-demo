...
import React, { useState, ReactNode, cloneElement, useEffect, ReactElement } from "react";
...
const Carousel: React.FC<CarouselProps> = ({ children }) => {
  ...
  const handleNext = () => {
    setCurrentIndex((prevIndex) => (prevIndex === children.length - 2 ? children.length - 2 : prevIndex + 1));
  };
  ...
  return (
    <div className="overflow-hidden relative w-full my-24 flex flex-col" style={{ position: "relative", width: "100%", overflow: "hidden" }}>
      <section role="navigation" className="flex justify-center gap-10 mt-16 order-last">
        <button onClick={handlePrev}>
          <svg
            ...
            className={`${currentIndex === -1 ? "text-mischka" : "text-vulcan"}`}
          >
        ...
        <button onClick={handleNext}>
          <svg width="48" height="49" viewBox="0 0 48 49" fill="none" xmlns="http://www.w3.org/2000/svg" className={`${currentIndex === children.length - 2 ? "text-mischka" : "text-vulcan"}`}>
            <g clipPath="url(#clip0_5_4814)">
              <path
                id="Vector"
                fillRule="evenodd"
      ...
      <motion.div
        ...
        style={{
          width: `calc(${children.length * itemWidth}% + ${(children.length - 1) * 30}px)`,
          x: `calc(${(currentIndex + 1) * -itemWidth + (100 - itemWidth) / 2}vw - 3rem)`,
          transition: "0.5s",
        }}
      >
        {children.map((child, index) => {
          const childEl = React.cloneElement(child as ReactElement, { active: currentIndex + 1 === index, color: currentIndex + 1 === index ? "blue" : "white" });
