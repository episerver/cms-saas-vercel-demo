...
export const SiteSearch : FunctionComponent<SiteSearchProps> = ({}) => {
    ...
    return <div ref={ container } className="quick-search-container">
        <MagnifyingGlassIcon className="quick-search-icon text-vulcan dark:text-white w-[44px] h-[44px] p-[5px]" onClick={() => setSearchBoxOpen(x => !x)} />
        { searchBoxOpen && <div className="quick-search-box border-solid border-[2px] absolute top-0 right-0 h-[56px] w-full rounded-[20px] bg-ghost-white dark:bg-vulcan z-[999] border-azure dark:border-verdansk flex flex-row justify-stretch content-stretch">
            <input type="text" className="rounded-l-[20px] w-[calc(100%-54px)] pl-[10px] focus:outline-none active:outline-none bg-ghost-white dark:bg-vulcan dark:text-ghost-white" placeholder="Search..." value={ quickSearchTerm } onChange={ e => setQuickSearchTerm(e.target.value) } />
            <MagnifyingGlassIcon className="text-azure dark:text-verdansk w-[52px] h-[52px] p-[9px]" onClick={() => setSearchBoxOpen(x => !x)} />
        </div> }
        { searchBoxOpen && (isLoading || data ) && <div className="quick-search-results border-solid border-[2px] border-azure dark:border-verdansk rounded-[20px] bg-ghost-white dark:bg-vulcan z-[999] absolute w-full min-h-[20px] top-[64px] right-0">
            ...
            { data && data.total > 0 && <ul className="p-[10px]">{data.items.slice(0,5).map(item => {
                return <li key={"qs-"+quickSearchTerm+"-"+item.id.key+item.id.locale}><Link href={ item.url ?? '#' } className="block w-full hover:text-azure dark:hover:text-verdansk">{ item.name }</Link></li>
            })}</ul> }
