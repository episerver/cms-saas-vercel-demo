...
export const LocationListPage : CmsComponent<GraphQL.LocationListPageDataFragment> = async ({ data, client, inEditMode, contentLink }) => 
{
    ...
    const imgSrcPath = data.image?.data?.path ?? data.image?.url
    ...
    return <div className='location-list-page'>
        <CmsContentArea fieldName='LocationsTopArea' items={ data.topArea ?? [] } client={ client } inEditMode={ inEditMode } />
        <div className='max-w-screen-2xl mx-auto'>
            <div className='intro border-b border-solid border-slate-800 w-full p-4 md:p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-2 grid-rows-2-first-auto gap-4 md:gap-6 lg:gap-8'>
                <h1 className='text-3xl font-semibold h-fit' data-epi-edit={ inEditMode ? "LocationListTitle" : undefined}>{ data.title ?? ""}</h1>
                ...
                <div className='rich-text' data-epi-edit={ inEditMode ? "LocationsIntroduction" : undefined} dangerouslySetInnerHTML={{ __html: data.intro ?? "" }}></div>
            </div>
            { (inEditMode || (data.mainArea ?? []).length > 0) && <CmsContentArea className="p-4 md:p-6 lg:p-8 pb-0 md:pb-0 lg:pb-0" fieldName='LocationsMainContent' items={ data.mainArea ?? [] } client={ client } inEditMode={ inEditMode } /> }
...
export const LocationListPageData = gql(/*GraphQL*/`fragment LocationListPageData on LocationListPage {
    ...
    topArea: LocationsTopArea {
        ...ContentAreaItemData
    }
    mainArea: LocationsMainContent {
        ...ContentAreaItemData
    }
}`)